...
void ResolveFixedArrayGet(ZilchSpirVFrontEnd* translator, Zilch::FunctionCallNode* functionCallNode, Zilch::MemberAccessNode* memberAccessNode, ZilchSpirVFrontEndContext* context)
{
  ...
  IZilchShaderIR* accessChainOp = translator->BuildCurrentBlockAccessChain(elementType, selfInstance, indexOperand, context);
...
void ResolveRuntimeArrayCount(ZilchSpirVFrontEnd* translator, Zilch::FunctionCallNode* functionCallNode, Zilch::MemberAccessNode* memberAccessNode, ZilchSpirVFrontEndContext* context)
{
  ...
  IZilchShaderIR* uintLengthResult = translator->BuildCurrentBlockIROp(OpType::OpArrayLength, uintType, structOwnerOp, zeroLiteral, context);
  // Cast to a signed int. Note: This will do very bad things if the sign bit is set on the unsigned int.
  IZilchShaderIR* intLengthResult = translator->BuildCurrentBlockIROp(OpType::OpBitcast, intType, uintLengthResult, context);
