...
Cog* LoadLauncherConfig(Cog* zeroConfigCog, const StringMap& arguments, bool overrideApplicationPath)
{
  ...
  String dataDirectory = FilePath::Combine(applicationDirectory, "Data");
  String toolsDirectory;

   //Build info is generated by the build process and if present the
  //engine was built locally.
  BuildInfo buildInfo;
  bool buildInfoExists = FindBuildInfo(applicationDirectory, buildInfo);
  if(buildInfoExists)
  {
    sourceDirectory = buildInfo.Source;
    dataDirectory = FilePath::Combine(sourceDirectory, "Data");
    toolsDirectory = FilePath::Combine(sourceDirectory, "Tools");
  }
  ...
  if(configCog == nullptr)
  {
    ...
    if(configCog == nullptr)
    {
      ...
      if(!FileExists(defaultConfig))
        defaultConfig = FilePath::Combine(dataDirectory, "DefaultLauncherConfiguration.data");
      if(FileExists(defaultConfig))
  ...
  mainConfig->mLocallyBuilt = buildInfoExists;
  ...
  if(buildInfoExists)
  {
    ...
    contentConfig->ToolsDirectory = FilePath::Combine(sourceDirectory, "Tools");
  }
