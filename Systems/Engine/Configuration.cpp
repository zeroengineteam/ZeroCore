...
Cog* LoadConfig(StringParam applicationName, bool useDefault, ZeroStartupSettings& settings)
{
  ...
  String dataDirectoryPath = cDataDirectoryName;
  ...
  if(buildInfoExists)
  {
    dataDirectoryPath = FilePath::Combine(buildInfo.Source, cDataDirectoryName);
  ...
  String searchConfigPaths [] =
  {
    ...
    FilePath::Combine(dataDirectoryPath, cDefaultConfigFile),
  };
  ...
  mainConfig->DataDirectory = dataDirectoryPath;
...
bool FindBuildInfo(String applicationDirectory, BuildInfo& outBuildInfo)
{
  ...
  for(size_t i = 0; i < buildInfoSearchPaths.Size(); ++i)
  {
    ...
    if(buildInfoExists)
    {
      ...
      LoadFromDataFile(outBuildInfo, buildInfoFile);
      return true;
    }
  }
  return false;
